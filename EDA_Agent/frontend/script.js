...
createApp({
    ...
    methods: {
        ...
        handleAttachSelect(event) {
            ...
            this.attachedFile = {
                name: file.name,
                type: file.name.endsWith('.pdf') ? 'PDF' : file.name.match(/\.(doc|docx)$/i) ? 'Word' : 'Excel'
            };
            this.attachUploadProgress = '0%';
            ...
            xhr.upload.onprogress = (e) => {
                if (e.lengthComputable) {
                    ...
                    this.attachUploadProgress = pct < 100 ? `${pct}%` : '处理中...';
                }
            };

            xhr.onload = () => {
                if (xhr.status >= 200 && xhr.status < 300) {
                    this.attachUploadProgress = '✓ 已上传';
                    setTimeout(() => { this.attachUploadProgress = ''; }, 2000);
                } else {
                    let msg = '上传失败';
                    try { msg = JSON.parse(xhr.responseText).detail || msg; } catch {}
                    this.attachUploadProgress = msg;
                    setTimeout(() => { this.attachedFile = null; this.attachUploadProgress = ''; }, 3000);
                }
                if (this.$refs.attachInput) this.$refs.attachInput.value = '';
            };

            xhr.onerror = () => {
                this.attachUploadProgress = '网络错误';
                setTimeout(() => { this.attachedFile = null; this.attachUploadProgress = ''; }, 3000);
                if (this.$refs.attachInput) this.$refs.attachInput.value = '';
            };
