...
# MilvusManager 和 EmbeddingService 的实例可以在整个应用中共享，避免重复初始化
milvus_manager = MilvusManager()
embedding_service = EmbeddingService()
# MilvusWriter 依赖 EmbeddingService 和 MilvusManager，所以在创建 MilvusWriter 实例时传入它们
...
# 聊天（非流式）入参 `message`、`user_id`、`session_id`。
...
# 聊天（流式 SSE）入参 `message`、`user_id`、`session_id`。
