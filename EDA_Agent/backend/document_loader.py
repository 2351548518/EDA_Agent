...
class DocumentLoader:
    ...
    def _split_page_to_three_levels(
        ...
        """
        这段代码实现了文档的多级分块（chunking）处理，将原始文本逐层细分为不同粒度的片段，并为每个片段分配唯一的标识和层级信息。

        首先，代码通过 self._splitter_level_1 对原始文本 text 进行一级分块，得到 level_1_docs。随后，依次遍历每个一级分块，提取其文本内容（去除空白），如果内容为空则跳过。每个一级分块会生成一个唯一的 chunk_id，并构建一个包含基础信息（base_doc）、文本内容、分块层级、索引等字段的字典，加入 root_chunks 列表。

        接着，对每个一级分块的文本再次用 self._splitter_level_2 进行二级分块，处理方式类似：为每个二级分块分配唯一 id，记录父分块 id（parent_chunk_id）、根分块 id（root_chunk_id）、层级等信息，并加入 root_chunks。

        最后，对每个二级分块的文本用 self._splitter_level_3 进行三级分块。每个三级分块同样会生成唯一 id，记录父分块和根分块的 id 关系，并将所有信息存入 root_chunks。

        这种分层分块的方式，便于后续对文档进行多粒度检索、追踪和重组，同时 chunk_id 的设计保证了每个片段在整个文档体系中的唯一性和可追溯性。
        """
